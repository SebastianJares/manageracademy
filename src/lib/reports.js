import { ARCHETYPES, PREFERENCES, SKILLS, TRAITS } from '../data/questions';
...
export const buildAnalysisHtml = (profile) => {
  ...
  const preferenceRows = Object.entries(profile.preferenceScores)
    .sort(([, a], [, b]) => b - a)
    .map(([key, score]) => `<div class="preference"><span><i style="background:${PREFERENCES[key].color}"></i>${escapeHtml(PREFERENCES[key].short)}</span><div><b style="width:${score}%;background:${PREFERENCES[key].color}"></b></div><strong>${score}</strong></div>`).join('');
...
<head>
  ...
  <style>
    ...
    .preferences{display:grid;gap:12px;margin-top:20px}.preference{display:grid;grid-template-columns:150px 1fr 36px;align-items:center;gap:10px;font-size:13px}.preference>span{display:flex;align-items:center;gap:7px}.preference i{width:9px;height:9px;border-radius:50%}.preference>div{height:7px;overflow:hidden;background:#dbe5ef;border-radius:10px}.preference b{display:block;height:100%;border-radius:inherit}.compass{border-top:4px solid #7658e8}.compass-note{margin-top:16px;padding:14px;border-left:3px solid #29b88f;background:#f1f8f6}.subtle{font-size:12px;color:#68778a}@media(max-width:720px){.preference{grid-template-columns:105px 1fr 30px;font-size:11px}}
  </style>
</head>
<body>
<main class="page">
  ...
  <div class="content">
    ...
    <section class="section">
      <h2>2. Manažerský kompas</h2>
      <p class="lead">Tato vrstva nehodnotí schopnosti. Ukazuje relativní pořadí šesti manažerských priorit uvnitř vašeho vlastního profilu.</p>
      <article class="compass">
        <h3>${escapeHtml(analysis.preference.title)}</h3>
        <div class="two">
          <div><strong>Nejsilnější preference · ${escapeHtml(analysis.preference.top.label)}</strong><p>${escapeHtml(analysis.preference.top.description)}</p></div>
          <div><strong>Doplňující preference · ${escapeHtml(analysis.preference.second.label)}</strong><p>${escapeHtml(analysis.preference.second.description)}</p></div>
        </div>
        <div class="preferences">${preferenceRows}</div>
        <div class="compass-note"><strong>${escapeHtml(analysis.preference.alignmentLabel)}</strong><p>${escapeHtml(analysis.preference.alignmentText)}</p></div>
        <p><strong>Méně přirozená priorita · ${escapeHtml(analysis.preference.low.label)}:</strong> ${escapeHtml(analysis.preference.low.low)}</p>
        <p class="risk"><strong>Možný stín dominantní preference:</strong> ${escapeHtml(analysis.preference.watchout)}</p>
        <p class="subtle">${escapeHtml(analysis.preference.note)}</p>
      </article>
    </section>
    <section class="section">
      <h2>3. Osobnostní kontext</h2>
    ...
    <section class="section">
      <h2>4. Mapa manažerských dovedností</h2>
    ...
    <section class="section">
      <h2>5. Pravděpodobné silné stránky</h2>
    ...
    <section class="section">
      <h2>6. Pravděpodobná rozvojová místa</h2>
    ...
    <section class="section">
      <h2>7. Vzorce, které stojí za ověření</h2>
    ...
    <section class="section">
      <h2>8. Doporučený začátek · ${escapeHtml(analysis.priority.label)}</h2>
  ...
  <footer class="footer">Manažerská akademie · soukromý sebereflexní výstup · verze metodiky 3</footer>
</main>
